...
export const OfferTime = ({ published, updated }: { published: Date, updated: Date }) => {
  const isUpdated = published !== updated
  const date = isUpdated ? new Date(updated) : new Date(published)
...
export const CardOffer = ({ offer }: { offer: Offer }) => {
  ...
  return (
    ...
    <div onClick={() => router.push(`/offer/${offer.id}`)} className='flex flex-row items-center cursor-pointer min-h-[200px] max-h-[250px] bg-white border border-gray-200 rounded-lg shadow lg:w-full md:flex-row lg:max-w-2xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 '>
      ...
      <div className='flex flex-col p-4 leading-normal w-full h-full grow md:self-start'>
        ...
        <p className=' text-xs font-normal text-gray-700 dark:text-gray-300 flex items-center gap-2'><span> {city} </span>  |  <span>{offer.teleworking}</span> | <OfferTime published={offer.published} updated={offer.updated} /> </p>
