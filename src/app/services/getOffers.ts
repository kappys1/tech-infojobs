import { CredentialsInvalidError } from '../constants/errors'
import {
  APIOfferReturn,
  APIResultOffers,
  APIResultsOffersError,
  Offer,
} from '../model/offer'
...
export async function getOffers(page = '1', queryParams: QueryParams = {}) {
  ...
  const responseJson: APIResultOffers & APIResultsOffersError = await res.json()
...
export async function getInfoJobsOffers({
...
}): Promise<APIOfferReturn> {
  const { items, currentPage, totalPages, totalResults, error } =
    await getOffers(page, queryParams)
  if (error === '103') {
  ...
  const listOfOffers: Offer[] = await Promise.all(
